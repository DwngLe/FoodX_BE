...
public class UserController {
    ...
    public APIResponse<Void> updateUserAvatar(@RequestParam MultipartFile multipartFile, @PathVariable UUID idUser)
            throws IOException {
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(description = "Không tìm thấy kết quả", responseCode = "404"),
                    @ApiResponse(description = "Không có quyền truy cập hoặc Token không hợp lệ", responseCode = "403"),
                    @ApiResponse(description = "Yêu cầu nộp bằng chứng", responseCode = "405"),
                    @ApiResponse(description = "Xác thực không thành công", responseCode = "401")
            })
    @PostMapping("/claimBusiness/{idRestaurant}")
    public APIResponse<Void> addBusinessProof(
